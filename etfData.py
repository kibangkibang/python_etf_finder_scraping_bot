...
if __name__ == "__main__":
    ...
    othersTitle = []
    baseUrl = 'https://www.etf.com/modal_forms/nojs/login?destination=/etfanalytics/etf-finder/'
    ...
    #로그인
    driver.get(url)
    name_txt = driver.find_element(by=By.ID, value='edit-name')
    name_txt.send_keys(config.etf_id)
    name_pass = driver.find_element(by=By.ID, value='edit-pass')
    name_pass.send_keys(config.etf_pass)
    btn_submit = driver.find_element(by=By.ID, value='edit-submit')
    btn_submit.submit()
    ...
    others = driver.find_elements(by=By.CSS_SELECTOR,value='li.Tooltip_default')
    for i in others:
        othersTitle.append(i)

    #btn = driver.find_elements_by_css_selector('button.inactiveResult')
    btn = driver.find_elements(by=By.CSS_SELECTOR,value='button.inactiveResult')
    btn[2].send_keys(Keys.ENTER)

    for num, i in enumerate(othersTitle):
        print(i.text)
        #btnnext = driver.find_element_by_id('nextPage')
        btnnext = driver.find_element(by=By.ID,value='nextPage')

        html = driver.page_source
        soup = BeautifulSoup(html)
        file_path_title = '/Users/sunghwanchoi/development/python/ETF/' + i.find_element(by=By.TAG_NAME,value='span').text.replace(" ", "") + '.txt'
                    ...
                    try:
                        if (num == 0 and col == 4):
                            ...
                            if (td.get_text().strip()[-1] == 'B'):
                                ...
                                col == 2 or col == 3 or col == 4 or col == 5 or col == 6 or col == 7 or col == 8)):
                            p = re.search('[+-]?[0-9]+(.[0-9]+)?', re.sub('[$,%]', '', td.get_text().strip()))
                            etf_data_file.write(str(p.group()) + "|")
        ...
        goToInput = driver.find_element(by=By.ID,value='goToPage')
        goToInput.clear()
        ...
        btnother = driver.find_element(by=By.CSS_SELECTOR,value=tabs)
